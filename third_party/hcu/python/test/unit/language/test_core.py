...
@pytest.mark.parametrize("in_dtype", [tl.float8e4nv, tl.float8e5])
...
def test_noinline(mode, device):
    ...
    if mode == "shared":
        z = torch.ones((32, 32), device=device, dtype=torch.float32)
    else:
    ...
    if mode == "simple":
        ...
        ref = torch.full((32, 32), 32, device=device, dtype=torch.float32)
...
@pytest.mark.parametrize("arch", ["gfx928", "gfx936", "gfx938"])
...
def test_override_arch(arch, env_var_override, device, fresh_knobs):
    ...
    if is_cuda():
        ...
        amdgcn_gfx = re.search(r'.amdgcn_target "amdgcn-amd-amdhsa--([^":-]+)', h.asm["amdgcn"])
