...
class MACABackend(BaseBackend):
    ...
    def make_ttgir(mod, metadata, opt, capability):
        ...
        inner_stages = opt.inner_stages
        if len(inner_stages) != 2:
            assert False, "len(inner_stages) must be equal to 2, please check config."
        ...
        if enable_dist:
            distributed.passes.ttir.add_convert_to_ttgpuir_ext(pm, f"cuda:{capability}", opt.num_warps, 64,
                                                               opt.num_ctas)
    ...
    def make_mlir(src, metadata, options, capability):
        ...
        if enable_dist:
            distributed.passes.ttgpuir.metax.add_distributed_to_llvm(pm, capability)
            metax.passes.ttgpuir.add_builtin_func_to_llvmir(pm)
    ...
    def make_mcfatbin(src, metadata, opt, capability):
        ...
        if (opt.pipeline == "basic" or opt.pipeline == "basic-prefetch") and "mla" not in scenarios:
        ...
        elif (opt.pipeline == "cpasync" or opt.pipeline == "cpasync-mixed") and "mla" not in scenarios:
            ...
            if "unroll" in scenarios:
                if opt.pipeline == "cpasync-mixed":
                    compile_options += " -mllvm -metaxgpu-mma-unroll-count=" + str(opt.num_stages + 1) + " "
                else:
                    compile_options += " -mllvm -metaxgpu-mma-unroll-count=" + str(opt.num_stages) + " "
            if "outerStage2" in scenarios:
        ...
        if llvm_version == 19:
            ...
            if "disable-live-range-split" in scenarios:
