...
PLUGIN_EXPORT void init_triton_ir(py::module &&m) {
  ...
  py::enum_<RoundingMode>(m, "ROUNDING_MODE", py::module_local())
      ...
      .value("RTNE_NO_NAN", RoundingMode::RTNE_NO_NAN);
  ...
  builderClass.def(py::init<MLIRContext *>())
      ...
      .def("create_inline_intrinsic",
           [](TritonOpBuilder &self, const std::string &inlineIntrinsic,
              const std::vector<Value> &values, const std::vector<Type> &types,
              bool isPure) -> OpState {
             return self.create<ElementwiseInlineIntrinsicOp>(
                 types, inlineIntrinsic, isPure, values);
           })
      .def("create_print",
...
PLUGIN_EXPORT void init_triton_env_vars(py::module &m) {
