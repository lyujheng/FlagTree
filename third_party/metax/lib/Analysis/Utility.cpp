...
#ifdef USE_MACA
bool supportMMA(triton::DotOp op, int major, int minor) {
  // Refer to mma section for the data type supported by Volta and Hopper
  // Tensor Core in
  // https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#warp-level-matrix-fragment-mma-884-f16
  auto aElemTy = op.getA().getType().getElementType();
  ...
  return supportMMA(op.getA(), major, minor) &&
         supportMMA(op.getB(), major, minor);
}

bool supportMMA(Value value, int major, int minor) {
  assert(major == 2 && "Unexpected MMA layout version found");
  auto elemTy = cast<TensorOrMemDesc>(value.getType()).getElementType();
  return elemTy.isF16() || elemTy.isBF16() || (elemTy.isF32() && major >= 2) ||
         (elemTy.isF64() && major >= 2) ||
         (elemTy.isInteger(8) && major >= 2) ||
         (llvm::isa<Float8E4M3FNType, Float8E5M2Type>(elemTy) && major >= 2 &&
          minor >= 6);
}
#endif
