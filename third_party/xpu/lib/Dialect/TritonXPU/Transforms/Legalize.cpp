...
struct TritonXPULegalizePass
    ...
    for (auto allOpTree : allOpTrees) {
      ...
      for (auto op : allOpTree) {
        ...
        if (auto rangeOp = dyn_cast<triton::MakeRangeOp>(op)) {
          ...
          // }
          auto expandDimsOps =
              findAllTypeUserOps<triton::ExpandDimsOp>(rangeOp);
  ...
  void filterOpChainSameRegion(llvm::SetVector<mlir::Operation *> &opChain,
                               mlir::Operation *anchorOp) {
    if (!anchorOp) {
      opChain.clear();
      return;
    }
    mlir::Region *baseRegion = anchorOp->getParentRegion();
    if (!baseRegion) {
      opChain.clear();
      return;
    }
    ...
    toErase.reserve(opChain.size());

    for (mlir::Operation *op : opChain) {
      if (!op || op->getParentRegion() != baseRegion) {
  ...
  bool canHoistTo(Operation *op, Operation *insertBefore,
    ...
    for (mlir::Value v : op->getOperands()) {
      if (auto ba = llvm::dyn_cast<mlir::BlockArgument>(v)) {
        if (!dom.dominates(ba.getOwner(), insertBefore->getBlock()))
          return false;
      } else {
        Operation *defOp = v.getDefiningOp();
        if (!defOp)
          return false;
        if (!dom.dominates(defOp, insertBefore))
          return false;
      }
    }
    return true;
  ...
  void runOnOperation() override {
    ...
    m.walk([&](triton::ReduceOp redOp) {
      LLVM_DEBUG(llvm::dbgs() << "reduceOp: \n" << redOp);
      auto axis = redOp.getAxis();
      auto shape =
          cast<RankedTensorType>(redOp.getSrcs()[0].getType()).getShape().vec();

      LLVM_DEBUG(llvm::dbgs() << "axis: " << axis << "\n");
      LLVM_DEBUG(llvm::dbgs() << "shape: " << shape.size() << ":");
      for (auto dim : shape) {
        LLVM_DEBUG(llvm::dbgs() << "shape " << dim << " ,");
      }
      LLVM_DEBUG(llvm::dbgs() << "\n");
          ...
          if (auto resTensorTy = mlir::dyn_cast<RankedTensorType>(resTy)) {
            ...
            if (auto resEncoding =
              ...
              auto newEncoding = triton::gpu::SliceEncodingAttr::get(
                  context, resEncoding.getDim(),
                  cast<triton::gpu::DistributedEncodingTrait>(
                      static_cast<Attribute>(newGlobalEncoding)));
    ...
    m.walk([&](triton::xpu::GM2LMOp gm2lmOp) {
      ...
      if (gm2lmOp.getCache()) {
        ...
        filterOpChainSameRegion(lmCacheChain, gm2lmOp);
    ...
    for (int i = lmCacheChains.size() - 1; i >= 0; --i) {
      ...
      for (auto op : lmCacheChain) {
        ...
        auto func = cast<triton::FuncOp>(ancestorOp);
