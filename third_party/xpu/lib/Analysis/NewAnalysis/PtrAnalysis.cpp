...
#include "triton/Analysis/NewAnalysis/PtrAnalysis.h"
...
MemRefType PtrState::getResultMemrefType(MLIRContext *context, int64_t offset,
  ...
  AffineMap map =
      makeStridedLinearLayoutMap(staticStrides, offset, source.getContext());
  // auto layout = AffineMapAttr::get(map);
  return MemRefType::get(resultShape, elementType, map, 1);
}
...
PtrState::createStackedCastOps(ArrayRef<int64_t> resultShape,
  ...
  //      | clampedOff
  //      *
  //      | nextOff (might overflow)
  //
  //    wrappedAroundOff = targetOffset % cols
  //    y = targetOffset / cols
  //    nextOff = y + rowSize
  //    clampedOff = min(nextOff, rows)
  //    d1 = clampedOff - y
  ...
  Value modRow = modulos[0]->size;
  if (isa<IntegerType>(modRow.getType())) {
    modRow = rewriter.create<arith::IndexCastOp>(loc, rewriter.getIndexType(),
                                                 modulos[0]->size);
  }
  ...
  Value y = rewriter.create<arith::DivSIOp>(loc, targetOffset, strideRow);
  Value nextOff = rewriter.create<arith::AddIOp>(loc, y, rowSize);
  Value clampedOff = rewriter.create<arith::MinSIOp>(loc, nextOff, modRow);
  Value d1 = rewriter.create<arith::SubIOp>(loc, clampedOff, y);
  ...
  d2 = rewriter.create<arith::MinSIOp>(loc, modRow, d2);
...
PtrState::createSideBySideCastOps(ArrayRef<int64_t> resultShape,
  ...
  Value modN = modulos[1]->size;
  if (isa<IntegerType>(modN.getType())) {
    modN = rewriter.create<arith::IndexCastOp>(loc, rewriter.getIndexType(),
                                               modulos[1]->size);
  }
  ...
  d2 = rewriter.create<arith::MinSIOp>(loc, modN, d2);
...
void PtrAnalysis::visitOperandExt(
    arith::ExtSIOp extOp, PtrState &state, const Location loc,
    ConversionPatternRewriter &rewriter,
    const llvm::SmallDenseMap<Value, PtrState> &knownPtrs) {
  visitOperand(extOp.getInMutable().get(), state, loc, rewriter, knownPtrs);
}
...
void PtrAnalysis::visitOperand(
  ...
  if (auto op = operand.getDefiningOp<arith::AddIOp>()) {
    ...
    visitOperandExt(op, state, loc, rewriter, knownPtrs);
