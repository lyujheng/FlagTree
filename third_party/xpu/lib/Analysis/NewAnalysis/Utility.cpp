...
std::map<unsigned, std::unique_ptr<redSMOffsetInfo>>
    ReduceOpHelper::reduceSMOffsetMap;
...
bool ReduceOpHelper::isCoreSynchronous() {
...
unsigned ReduceOpHelper::getIntraGroupSizeWithUniqueData() {
  auto srcReduceDimSize = static_cast<unsigned>(srcShape[axis]);
  unsigned elementPerThreads =
      triton::xpu::getUniqueContigPerCore(getSrcLayout(), getSrcShape())[axis];
  return std::min(srcReduceDimSize / elementPerThreads,
                  triton::xpu::getCoresPerGroupWithUniqueData(
                      getSrcLayout(), getSrcShape())[axis]);
}

SmallVector<unsigned> ReduceOpHelper::getXPUScratchConfig() {
...
unsigned ReduceOpHelper::getXPUScratchSizeInBytes() {
  auto smemShape = getXPUScratchConfig();
...
bool ScanLoweringHelper::isCoreSynchronous() { return false; }
...
triton::xpu::ClusterLayoutAttr ScanLoweringHelper::getXPUEncoding() {
  return cast<triton::xpu::ClusterLayoutAttr>(srcEncoding);
}
...
unsigned ScanLoweringHelper::getScratchSizeInElemsXPU() { return 64; }
