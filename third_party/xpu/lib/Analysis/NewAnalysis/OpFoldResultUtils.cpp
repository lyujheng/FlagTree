...
bool hasConstOne(const OpFoldResult ofr) {
  auto intAttr = getIntAttr(ofr);
  if (intAttr.has_value())
    return intAttr.value() == 1;
  ...
  intAttr = getIntAttr(constOp.getValue());
  return intAttr.has_value() && intAttr.value() == 1;
}
...
// Keep these triton_shared OpFoldResult helpers in XPU NewAnalysis so the
// regenerated libTritonSharedForXPU.a can depend on libTritonXPUAnalysis.a
// instead of linking its own OpFoldResultUtils.cpp.o, which duplicates the
// helper symbols above.
// ==================== FLAGTREE XPU SYNC MARK ====================
...
OpFoldResult compareOFRs(const OpFoldResult lhs, const OpFoldResult rhs,
  ...
  return b.create<arith::SelectOp>(loc, cmpOp, trueValue, falseValue)
      .getResult();
}

std::pair<Value, Value> modRowDiv(Value modRow, Value strideRow,
                                  const Location loc, OpBuilder &b) {
  if (auto mulOp = modRow.getDefiningOp<arith::MulIOp>()) {
    OpBuilder::InsertionGuard guard(b);
    b.setInsertionPointAfter(mulOp);
    Value lhs =
        b.create<arith::IndexCastOp>(loc, b.getI64Type(), mulOp.getLhs());
    Value rhs =
        b.create<arith::IndexCastOp>(loc, b.getI64Type(), mulOp.getRhs());
    modRow = b.create<arith::MulIOp>(loc, lhs, rhs);

    Value i64StrideRow =
        b.create<arith::IndexCastOp>(loc, b.getI64Type(), strideRow);
    Value divRes = b.create<arith::DivSIOp>(loc, modRow, i64StrideRow);
    divRes = b.create<arith::TruncIOp>(loc, b.getI32Type(), divRes);
    divRes = b.create<arith::IndexCastOp>(loc, b.getIndexType(), divRes);
    return {modRow, divRes};
  }

  Value divRes = b.create<arith::DivSIOp>(loc, modRow, strideRow);
  return {modRow, divRes};
}
