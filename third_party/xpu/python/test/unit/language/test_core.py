...
@pytest.mark.skipif(is_xpu(), reason="Skip for kunlunxin: not in active test set")
...
def _mod_operation_ill_conditioned(dtype_x, dtype_y) -> bool:
    ...
    return (dtype_x, dtype_y) in [
        ...
        ('int64', 'float64'),
        # ===-------------------- For Triton XPU -----------------------===
        # Triton XPU Can Pass This Dtype Combination
        # ('uint16', 'bfloat16'),
        # ('uint16', 'float16'),
        # ('uint16', 'float32'),
        # Triton XPU Cannot Pass This Dtype Combination
        ('int32', 'float64'),
        ('uint16', 'float64'),
        ('uint32', 'float64'),
        # ===-----------------------------------------------------------===
...
def test_bin_op(dtype_x, dtype_y, op, num_ctas, device):
    # XPU backend does not support float modulo (% with any float operand)
    _float_types = float_dtypes + ['bfloat16']
    if op == '%' and (dtype_x in _float_types or dtype_y in _float_types) and is_xpu():
        pytest.skip("Skip for kunlunxin: float modulo unsupported")
    expr = f' x {op} y'
...
def test_compare_op(dtype_x, dtype_y, op, mode_x, mode_y, num_ctas, device):
    if (mode_x == "nan" or mode_y == "nan") and is_xpu():
        pytest.skip("Skip for kunlunxin")
    expr = f'x {op} y'
    if (dtype_x in uint_dtypes and dtype_y in int_dtypes and _bitwidth(dtype_x) >= _bitwidth(dtype_y)):
        numpy_expr = f'x.astype(np.{dtype_x}) {op} y.astype(np.{dtype_x})'
    elif (dtype_y in uint_dtypes and dtype_x in int_dtypes and _bitwidth(dtype_y) >= _bitwidth(dtype_x)):
...
def test_where(dtype, num_ctas, device):
    if dtype == "float64" and is_xpu():
        pytest.skip("Skip for kunlunxin")
    if dtype in ("float32", "bfloat16") and is_xpu():
        pytest.skip("Skip for kunlunxin: LLVM assert 'Calling a function with a bad signature' in debug build")
    select_ptrs = False
    if dtype == "*int32":
        dtype = "int64"
        select_ptrs = True
...
def test_math_op(dtype_x, expr, x, device):
    if expr in ['exp2', 'log2'] and is_xpu():
        pytest.skip("Skip for kunlunxin")
    _test_unary(dtype_x, f'tl.{expr}({x})', f'np.{expr}({x}) ', device=device)


@pytest.mark.skipif(is_xpu(), reason="Skip for kunlunxin")
@pytest.mark.interpreter
...
def test_cast(dtype_x, dtype_z, bitcast, size, num_ctas, device):
    if (dtype_x == 'float64' or dtype_x == 'int8' and dtype_z == 'bfloat16') and is_xpu():
        pytest.skip("Skip for kunlunxin")
...
@pytest.mark.interpreter
@pytest.mark.parametrize("dtype_str", [dtype for dtype in torch_dtypes if dtype != 'int16'])
@pytest.mark.parametrize("num_ctas", num_ctas_list)
...
def test_reduce(op, dtype_str, shape, axis, keep_dims, num_ctas, device):
    if (dtype_str == 'float64' and op in ['min', 'max']) and is_xpu():
        pytest.skip("Skip for kunlunxin")
    check_type_supported(dtype_str, device)  # bfloat16 on cc < 80 will not be tested
...
@pytest.mark.skipif(True, reason="skip for no ir dump support")
...
def test_vectorization(N, num_ctas, device):
    ...
    if not is_cuda():
        # ===-------------------- For Triton XPU -----------------------===
        # llir may be unavailable when TRITON_STORE_BINARY_ONLY=True (desensitization)
        if "llir" in pgm.asm:
            llir = str(pgm.asm["llir"])
            if llir and N % 16 == 0:
                assert "16 x float" in llir
        # ===-----------------------------------------------------------===
        return
...
def test_bin_op_constexpr(op, is_lhs_constexpr, is_rhs_constexpr, device):
    # XPU backend does not support float modulo (% uses float32 in this test)
    if op == '%' and is_xpu():
        pytest.skip("Skip for kunlunxin: float modulo unsupported")
