...
class CompiledKernel:
    ...
    def __init__(self, src, metadata_group, hash):
        ...
        self.asm = AsmDict({
            file.suffix[1:]: file.read_bytes() if file.suffix[1:] in {binary_ext, "elf"} else file.read_text()
    ...
    def _init_handles(self):
        ...
        is_xpu_backend = getattr(self.metadata, "backend_name", None) == "xpu" or \
            hasattr(self.metadata, "is_sdnn")
        if is_xpu_backend:
            (self.module, self.function, self.n_regs, self.n_spills,
             self.n_max_threads) = driver.active.xpu_utils.load_binary(self.name, self.kernel, self.metadata.is_sdnn,
                                                                       self.metadata.printf_buf_offset)
        else:
            (self.module, self.function, self.n_regs, self.n_spills,
             self.n_max_threads) = driver.active.utils.load_binary(self.name, self.kernel, self.metadata.shared, device)
