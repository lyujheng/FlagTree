...
def _get_selected_backend() -> str:
    if selected_backend := os.environ.get("TRITON_BACKEND", ""):
        return selected_backend
    if selected_backend := os.environ.get("FLAGTREE_BACKEND", ""):
        return selected_backend
    try:
...
def _is_enabled_backend(name: str) -> bool:
    selected_backend = _get_selected_backend()
    if selected_backend:
        return name == selected_backend
    return True
...
def _discover_backends() -> dict[str, Backend]:
    ...
    if skip_entrypoints_env == "1":
        ...
        for name in os.listdir(root):
            ...
            if name.startswith('__'):
                continue
            if not _is_enabled_backend(name):
                continue
    ...
    for ep in entry_points().select(group="triton.backends"):
        if not _is_enabled_backend(ep.name):
            continue
