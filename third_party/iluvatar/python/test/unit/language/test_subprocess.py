...
import triton
from triton._internal_testing import is_corex, is_interpreter
...
def test_print(func_type: str, data_type: str, device: str):
    if is_corex() and func_type in ["device_print", "device_print_scalar"] and data_type == "float64":
        pytest.skip("CoreX torch.arange(...).to(float64) currently produces zeros")
