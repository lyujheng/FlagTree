...
import re
from triton._internal_testing import is_cuda, is_hip, is_hip_cdna3, is_hip_cdna4, is_hip_cdna, is_corex
...
def test_simple_matmul(dtype_src_str, dtype_dst_str, BLOCK_M, BLOCK_N, BLOCK_K, NUM_STAGES, NUM_WARPS, NUM_CTAS, device,
    ...
    check_type_supported(dtype_src_str, device)
    check_type_supported(dtype_dst_str, device)
    ...
    if is_corex() and dtype_dst_str == "float16":
        pytest.skip("test out_dtype=float16 is not supported on corex")
...
def test_mxfp(BLOCK_M, BLOCK_N, BLOCK_K, NUM_STAGES, nonKDim, NUM_WARPS, device):
    ...
    if (is_cuda() or is_corex()) and torch.cuda.get_device_capability()[0] < 10:
...
@pytest.mark.skipif(is_hip() and not is_hip_cdna4() or is_corex(),
