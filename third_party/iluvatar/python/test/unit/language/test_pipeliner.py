...
from triton._internal_testing import is_cuda, is_corex, is_hopper_or_newer, is_hip_cdna, is_hip_cdna2, is_hip
...
def test_pipeline_matmul(scale, device):
    check_capabilities()
    if scale and not (is_cuda() or is_corex() or is_hip_cdna()):
        pytest.skip("NYI: scale_dot just implemented in CUDA/HIP")
    M, N, K = 512, 512, 128
    BLOCK_M, BLOCK_N, BLOCK_K = 64, 64, 32
    NUM_STAGES = 4 if is_cuda() or is_corex() else 2
