...
def check_type_supported(dtype, device):
    ...
    if device in ['cuda']:
        ...
        if (cc[0] < 8 and not is_corex()) and (dtype is tl.bfloat16 or dtype == "bfloat16" or dtype is torch.bfloat16):
            pytest.skip("bfloat16 is only supported on NVGPU with cc >= 80")
        if cc[0] < 9 and dtype in {tl.float8e4nv, "float8e4nv", "float8_e4m3fn"}:
            pytest.skip("float8e4nv is only supported on NVGPU with cc >= 90")
        if is_corex() and (dtype is tl.float64 or dtype == "float64" or dtype is torch.float64):
            pytest.skip("float64 is not supported on corex")
        if is_corex() and (dtype in torch_float8_dtypes or dtype is torch.float8_e4m3fn or dtype is torch.float8_e5m2):
            pytest.skip("float8 is not supported on corex")
    if is_interpreter():
...
def test_precise_math(expr_prec, expr_ref, num_ctas, device):
    if is_corex() and "float64" in expr_ref:
        pytest.skip("float64 not supported on CoreX")
...
def test_atomic_cas(sem, num_ctas, dtype_str, device):
    ...
    if is_corex() and dtype_str == "int64":
        pytest.skip("CoreX does not support atomic cas with int64/uint64 types")
...
def test_tensor_atomic_cas(sem, size, dtype_str, num_ctas, device):
    ...
    if is_corex() and dtype_str in ['uint64', 'int64']:
        pytest.skip("CoreX does not support atomic cas with int64/uint64 types")
...
def get_test_dot_mixed_sizes_cases():
    ...
    available_precision = ["tf32" if is_cuda() or is_corex() else "ieee"]
    return [
...
def get_test_small_dots_cases():
    if not is_cuda() and not is_corex():
        return []
...
def test_dot(M, N, K, num_warps, col_a, col_b, epilogue, input_precision, in_dtype, out_dtype, kpack, mma_nonk_size,
    ...
    if is_interpreter():
        ...
        if is_cuda() or is_corex():
            ...
            if capability[0] < 8:
                ...
                if input_precision != "ieee" and not is_corex():
    ...
    if not (is_cuda() and not is_corex() or is_hip_cdna()):
        return
...
def test_dot3d(B, num_warps, M, N, K, BLOCK_M, BLOCK_N, in_dtype_str, out_dtype_str, device):
    if is_hip():
        ...
        input_precision = "tf32" if (is_cuda() or is_corex()) and in_dtype_str == 'float32' else "ieee"
...
def test_load_cache_modifier(cache, device):
    ...
    if is_corex():
        llir = pgm.asm['llir']
        load_line = [line for line in llir.splitlines() if "llvm.bi.load.kop" in line][0]
        expected_kop = {'.ca': 0, '.cg': 1, '.cv': 3}.get(cache, 0)
        assert f"i32 {expected_kop}" in load_line
        return
...
def test_poison_return(device):
    ...
    if not (is_hip() or is_xpu() or is_corex()):
...
def test_num_threads(device):
    ...
    @triton.jit
    def kernel(Out):
        num_threads: tl.constexpr = tl.extra.corex.num_threads()
...
def test_globaltimer(device):
    ...
    if is_hip() or is_corex():
        pytest.skip("test_globaltimer is flaky on AMD GPUs and is not supported on Iluvatar GPUs")
...
def test_smid(device):
    ...
    @triton.jit
    def kernel(Out):
        tl.store(Out + tl.program_id(0), tl.extra.corex.smid())
...
def test_dot_max_num_imprecise_acc(M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, in_type_str, low_precision_acc, device):
    ...
    if is_cuda() or is_corex():
        cc = torch.cuda.get_device_capability()
        if cc[0] >= 9 and in_type_str == "float8e4b15" or is_corex():
...
@pytest.mark.skipif(not is_cuda() and not is_corex(), reason="Requires CUDA")
...
def test_enable_reflect_ftz(enable_reflect_ftz, device, fresh_knobs):
    ...
    if not is_corex():
        ...
        found_ex2_ftz = re.search(r'ex2.approx.ftz.f32', h.asm["llir"]) is not None
...
@pytest.mark.parametrize("arch", ["sm70", "sm80", "sm90", "gfx942", "gfx950", "gfx1200", "sm71"])
@pytest.mark.parametrize("env_var_override", [False, True])
def test_override_arch(arch, env_var_override, device, fresh_knobs):
    if (arch == "sm71" and not is_corex()) or (is_corex() and arch not in ["sm71", "sm80"]):
        pytest.skip(f"CoreX only supports sm71 and sm80 now")
    if arch.startswith("sm") and not is_cuda() and not is_corex():
...
def test_maxnreg(device):
    ...
    if not is_interpreter():
        ...
        try:
            if is_corex():
                assert re.search(r'"iluvatar-num-vgpr"="42"', k.asm["llir"])
            else:
...
def test_dot_multidim(rank, trans_a, trans_b, device):
    ...
    if is_corex() and rank > 2:
        pytest.skip("FIXME: Iluvatar TCU only supports 2D dot; batched (>2D) dot is not supported yet")
