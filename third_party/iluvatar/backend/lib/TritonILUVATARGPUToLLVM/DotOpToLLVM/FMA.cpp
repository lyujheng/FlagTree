...
class ILUVATARFMAVectorMultiplier : public FMAVectorMultiplier {
    ...
    if (dotAvailable) {
      if ((aElemTy.isF16() || aElemTy.isBF16()) && dElemTy.isF32()) {
        ...
        chosenOp.intrinsicName =
            aElemTy.isF16() ? "llvm.bi.fdot2" : "llvm.bi.bfdot2";
      ...
      if (aElemTy.isInteger(8) && dElemTy.isInteger(32)) {
        ...
        chosenOp.intrinsicName = "llvm.bi.idot4";
        chosenOp.additionalArgs = {};
        return chosenOp;
      }
    }
...
public:
  ILUVATARFMAVectorMultiplier(ConversionPatternRewriter &rewriter, DotOp op)
...
LogicalResult convertILUVATARFMADot(DotOp op, DotOp::Adaptor adaptor,
                                    const LLVMTypeConverter *typeConverter,
                                    ConversionPatternRewriter &rewriter) {
  ILUVATARFMAVectorMultiplier multiplier(rewriter, op);
