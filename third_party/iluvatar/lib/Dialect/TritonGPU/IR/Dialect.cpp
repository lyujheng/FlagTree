...
Attribute parseSwizzledEncoding(AsmParser &parser, Type type) {
    ...
    if (attr.getName() == "vec") {
      ...
      if (parseIntArrayAttr(parser, attr, order, "order").failed())
        return {};
               ...
               std::is_same_v<SpecificEncoding, SwizzledSharedEncodingAttr>) {
  ...
  if (auto CTALayout = parseCTAAttr(parser, ctaAttr, order.size())) {
    if constexpr (std::is_same_v<SpecificEncoding,
                                 SwizzledSharedEncodingAttr>) {
      return parser.getChecked<SpecificEncoding>(parser.getContext(), vec,
...
SwizzledSharedEncodingAttr::verify(function_ref<InFlightDiagnostic()> emitError,
                                   ...
                                   CTAEncodingAttr ctaLayout, bool /*useTcu*/) {
  if (order.size() != ctaLayout.getRank()) {
...
#ifdef __ILUVATAR__
bool IluvatarMmaEncodingAttr::isVolta() const { return getVersionMajor() == 1; }

SmallVector<unsigned> IluvatarMmaEncodingAttr::getRepOrder() const {
  return getMatrixOrder(getWarpsPerCTA().size(), /*rowMajor*/ true);
}

SmallVector<unsigned>
IluvatarMmaEncodingAttr::getRepOrderForOperand(int opIdx) const {
  return getOrderForDotOperand(opIdx, getWarpsPerCTA().size(),
                               /*kContig*/ true);
}

SwizzledSharedEncodingAttr
IluvatarMmaEncodingAttr::composeSharedLayoutForOperand(
    CTAEncodingAttr ctaLayout, int /*operandIdx*/,
    ArrayRef<int64_t> /*operandShape*/, ArrayRef<unsigned> sharedOrder,
    unsigned /*kWidth*/, unsigned elemBitWidth, bool /*needTrans*/,
    unsigned /*useSme*/) const {
  // Mark Iluvatar dot shared memory explicitly. Ordinary shared layouts may
  // also use vec/perPhase/maxPhase = 1/1/1, but they must not accidentally use
  // the TCU/SME shared layout.
  //
  // The SME useTcu layout is element-bit-width specific (fp16/bf16 vs fp32 vs
  // int8 emit different SME byte patterns). We carry the bit width in `vec`,
  // which is also semantically accurate: the SME hardware moves 16 rows x 64
  // contiguous bytes, i.e. 512/bitwidth contiguous elements per segment, and
  // `vec` is exactly the contiguous vectorization granularity. So vec =
  // 64/32/16 for int8/fp16/fp32, and swizzledSharedToLinearLayout recovers the
  // width as 512/vec. This makes the (shape, encoding) cache key unique per bit
  // width without threading the width through toLinearLayout. maxPhase stays 1
  // so "is swizzled" checks still treat SME shared as non-swizzled.
  unsigned smeVec = elemBitWidth ? 512u / elemBitWidth : 1;
  return SwizzledSharedEncodingAttr::get(getContext(), /*vec=*/smeVec,
                                         /*perPhase=*/1, /*maxPhase=*/1,
                                         sharedOrder, ctaLayout,
                                         /*useTcu=*/true);
}

SmallVector<int64_t>
IluvatarMmaEncodingAttr::getRepForOperand(ArrayRef<int64_t> shape,
  ...
  assert(instrShape.size() == 3 && "Iluvatar TCU expects an M/N/K shape");

  auto ceilDiv = [](int64_t lhs, int64_t rhs) { return (lhs + rhs - 1) / rhs; };

  auto warpsPerCTA = getWarpsPerCTA();
  int64_t numRepBatch =
      rank == 3 ? std::max<int64_t>(1, shape[0] / warpsPerCTA[0]) : 1;
  unsigned mDim = rank - 2;
  unsigned nDim = rank - 1;
  int64_t mTile = instrShape[0] * warpsPerCTA[mDim];
  int64_t nTile = instrShape[1] * warpsPerCTA[nDim];
  int64_t kTile = instrShape[2];

  if (opIdx == 0) {
    return {numRepBatch, std::max<int64_t>(1, shape[mDim] / mTile),
            std::max<int64_t>(1, ceilDiv(shape[nDim], kTile))};
  }
  return {numRepBatch, std::max<int64_t>(1, ceilDiv(shape[mDim], kTile)),
          std::max<int64_t>(1, shape[nDim] / nTile)};
}
#endif
...
LogicalResult DotOperandEncodingAttr::verify(
  ...
  if (auto parentAttr = mlir::dyn_cast<IluvatarMmaEncodingAttr>(parent)) {
    if (!parentAttr.isVolta())
      return emitError() << "ttg.dot_op only supports Iluvatar MMA v1 for now";
    return success();
  }
#endif
...
struct TritonGPUInferLayoutInterface
  ...
  inferTransOpEncoding(Attribute operandEncoding, ArrayRef<int64_t> shape,
    ...
    if (auto enc = dyn_cast<SwizzledSharedEncodingAttr>(operandEncoding)) {
#ifdef __ILUVATAR__
      // The TCU SME swizzled-shared row-major (order[0]!=0) and col-major
      // (order[0]==0) forms are each fit to the SME rowxfb16/colxfb16 hardware
      // dump and are NOT exact transposes of each other.  The "swap order, keep
      // encoding" shortcut below therefore makes memdesc_trans non
      // round-tripping (e.g. chain-dot `dot(trans(dot(...)), ...)` computes
      // wrong results).  For useTcu, fall through to the generic
      // transposeLinearLayout path so the transposed view is the exact
      ...
      // useTcu == true: fall through to the generic transpose path below.
#endif
    }
