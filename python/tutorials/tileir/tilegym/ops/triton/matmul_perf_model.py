...
from triton.testing import get_max_tensorcore_tflops
...
def _get_clock_rate_in_khz():
    return nvsmi(["clocks.max.sm"])[0] * 1e3


def _get_tensorcore_tflops(device, num_ctas, num_warps, dtype):
    ...
    tflops = (min(num_subcores, total_warps) / num_subcores *
              get_max_tensorcore_tflops(dtype, _get_clock_rate_in_khz(), device))
    return tflops


def _get_simd_tflops(device, num_ctas, num_warps, dtype):
    """return compute throughput in TOPS"""
    total_warps = num_ctas * min(num_warps, 4)
    num_subcores = driver.active.utils.get_device_properties(device)["multiprocessor_count"] * 4  # on recent GPUs
    tflops = (min(num_subcores, total_warps) / num_subcores *
              get_max_simd_tflops(dtype, _get_clock_rate_in_khz(), device))
    return tflops


def _get_tflops(device, num_ctas, num_warps, dtype):
    capability = torch.cuda.get_device_capability(device)
    if capability[0] < 8 and dtype == torch.float32:
        return _get_simd_tflops(device, num_ctas, num_warps, dtype)
    return _get_tensorcore_tflops(device, num_ctas, num_warps, dtype)
...
def estimate_matmul_time(
    ...
    **kwargs,
    ...
    tput = _get_tflops(device, num_ctas, num_warps, dtype)
    ...
    logger.debug(f"Total time: {total_time_ms}ms, compute time: {compute_ms}ms, "
...
def early_config_prune(configs, named_args, **kwargs):
    ...
    if isinstance(named_args["A"], TensorDescriptor):
        dtsize = named_args["A"].base.element_size()
        dtype = named_args["A"].base.dtype
    else:
    ...
    if dtype not in [torch.float16, torch.float32]:
        # A missing "SPLIT_K" key means SPLIT_K == 1 (no split), so keep the config.
        configs = [config for config in configs if config.kwargs.get("SPLIT_K", 1) == 1]
